...
新安装默认使用实验性的 `deepseek-v4-flash-vision-exp` 模型，使图片输入可以通过你配置的 DeepSeek 兼容端点工作。可以参考 DeepSeek 的[视觉 API 指南](https://api-docs.deepseek.com/guides/vision)和[Files API 文档](https://api-docs.deepseek.com/guides/files_api)了解服务端限制。PDF 不会被直接发送：当前 Files API 只接受图片，因此 PDF 文本提取或页面转图暂未集成到棱镜中。
...
当你填写 API Key 并使用依赖模型的功能时，即表示你授权棱镜通过已配置的 API 端点，将对话内容、你主动发送的附件以及由此形成的用户画像数据传输给 DeepSeek。这些数据可能包括消息、图片数据、文本/代码文件内容、人物、情绪、记忆、盲点、叙事时间轴记录、摘要和搜索上下文。
