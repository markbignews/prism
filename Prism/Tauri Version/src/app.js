...
window.api = {
  ...
  getVersion: function() { return '1.0.16'; },
...
const I18N = {
  en: {
    ...
    flashModelOption: 'DeepSeek-V4-Flash-Vision-Exp（实验，默认）',
    ...
    dataNote: 'Conversation data and indexes are stored locally. Model-backed requests send the required context to the configured API.',
  ...
  zh: {
    ...
    defaultMode: '默认模式',
    dataStorage: '数据存储',
    dataNote: '对话数据和索引保存在本地。依赖模型的请求会把所需上下文发送到你配置的 API。',
    chapterDetail: '章节详情',
    summary: '摘要',
    jumpToSource: '定位原文',
    keywords: '关键词',
...
function attachmentMime(file) {
  ...
  const map = { png:'image/png', jpg:'image/jpeg', jpeg:'image/jpeg', gif:'image/gif', webp:'image/webp',
    ...
    html:'text/html', htm:'text/html', css:'text/css', js:'text/javascript', ts:'text/javascript',
...
async function readComposerAttachment(file) {
  ...
  const isText = TEXT_MIME_TYPES.has(mime) || mime.startsWith('text/') || TEXT_EXTENSIONS.has(ext);
  if (!isImage && !isText) throw new Error(t('attachmentUnsupported'));
  if (isImage && file.size > 32 * 1024 * 1024) throw new Error(t('attachmentTooLarge'));
  if (isText && file.size > 2 * 1024 * 1024) throw new Error(t('attachmentTooLarge'));
  const buffer = await file.arrayBuffer();
  if (isImage) {
    return { id: attachmentId(), fileName: file.name, mediaType: mime, kind: 'image',
      dataBase64: base64FromBuffer(buffer), textContent: '' };
  }
  return { id: attachmentId(), fileName: file.name, mediaType: 'text/plain', kind: 'text',
    dataBase64: '', textContent: new TextDecoder('utf-8', { fatal: false }).decode(buffer) };
}
...
function renderAttachmentStrip() {
  ...
  strip.hidden = S.draftAttachments.length === 0;
  S.draftAttachments.forEach(attachment => {
    ...
    if (attachment.kind === 'image' && attachment.dataBase64) {
      ...
      chip.innerHTML = iconSvg('archive', 'ui-icon')
        + '<span class="attachment-chip-name" title="' + esc(attachment.fileName) + '">' + esc(attachment.fileName) + '</span>';
    }
    ...
    remove.onclick = () => {
      S.draftAttachments = S.draftAttachments.filter(item => item.id !== attachment.id);
      renderAttachmentStrip();
      updateSendBtn();
    };
...
async function addComposerFiles(files) {
  ...
  for (const file of files.slice(0, Math.max(0, 5 - S.draftAttachments.length))) {
    try {
      S.draftAttachments.push(await readComposerAttachment(file));
    } catch (error) {
      showErrorBanner(errMsg(error));
    }
  }
  renderAttachmentStrip();
  updateSendBtn();
...
function getObText(key) {
  const translations = {
    en: {
      ...
      privacyBody: "Conversations, indexes, and analysis records are stored locally, but model-backed features send the required context and related data to the model API you choose.",
    ...
    zh: {
      ...
      choosePath: "选择本地数据目录...",
      privacyTitle: "数据与隐私",
      privacyBody: "对话、索引和分析档案保存在本地，但依赖模型的功能会把所需上下文和相关数据发送到你选择的模型 API。",
      getStarted: "开始使用",
      saving: "正在保存…",
