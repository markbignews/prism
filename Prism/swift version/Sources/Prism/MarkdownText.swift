    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ForEach(Array(blocks.enumerated()), id: \.offset) { _, block in
                switch block {
                case .blank:
                    // A fixed blank-line gap must not absorb extra height when
                    // a sibling (such as the reasoning disclosure) expands.
                    Color.clear
                        .frame(height: 4)
        ...
        // Keep paragraph spacing intrinsic so only the reasoning panel changes
        // height during disclosure animations.
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ...
            if isHovering || copied {
                Button {
                    NativeSupport.copyToClipboard(content)
                    copied = true
                ...
                .buttonStyle(.plain)
                .help(L10n.text(.copyCode, settings.language))
                .padding(4)
            }
        }
        .onHover { hovering in
